...
class TeacherProfileManager:
    ...
    def save_profile(self, profile: TeacherProfile) -> None:
        ...
        Args:
            profile: 教师画像
        """
        profile_file = os.path.join(self.persist_directory, f"profile_{profile.teacher_id}.json")
    ...
    def load_profile(self, teacher_id: str) -> Optional[TeacherProfile]:
        ...
        Returns:
            教师画像，如果没有找到则返回None
        """
        profile_file = os.path.join(self.persist_directory, f"profile_{teacher_id}.json")
        ...
        try:
            ...
            styles = [TeachingStyle(**style_data) for style_data in data.get("teaching_styles", [])]
        ...
        for filename in os.listdir(self.persist_directory):
            if filename.startswith("profile_") and filename.endswith(".json"):
                ...
                try:
                    ...
                    logger.warning(f"读取画像文件失败 ({profile_path}): {e}")
                    continue
        return profiles
...
class TeacherStyleVectorStore:
    """教师风格向量存储（基于 ChromaDB 的内存实现）

    当前使用内存列表存储，未来可升级为 ChromaDB 持久化向量检索。
    """

    def __init__(
        self, embedding_model: Embeddings = None, persist_directory: str = "./data/style_vectors"
    ):
        self.embedding_model = embedding_model
        self.persist_directory = persist_directory
        self._styles: List[Dict[str, str]] = []

    def add_style(self, teacher_id: str, style_name: str, style_description: str) -> None:
        """添加教师风格向量（内存存储，未来升级为 ChromaDB）"""
        self._styles.append(
            {
                "teacher_id": teacher_id,
    ...
    def add_teacher_style(self, teacher_id: str, style_description: str) -> None:
        """添加教师风格向量"""
        self.add_style(teacher_id, "未命名风格", style_description)

    def find_similar_teachers(self, style_description: str, k: int = 5) -> List[str]:
        """查找风格相似的教师（当前返回空列表，未来实现向量相似度检索）"""
        return []

    def find_similar_styles(self, style_description: str, k: int = 5) -> List[Dict]:
        """查找风格相似的教学风格（当前返回空列表，未来实现向量相似度检索）"""
        return []

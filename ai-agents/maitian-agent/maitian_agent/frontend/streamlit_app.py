...
Agent 创建统一通过 AgentFactory.create_all()，确保 RAG/记忆/工具依赖注入生效。
"""
...
# Agent 类型 → session_state 键名的映射
_AGENT_KEY_MAP: dict[str, str] = {
    "quick_lesson_prep": "quick_lesson_prep_agent",
    "wisdom_transfer": "wisdom_transfer_agent",
    "classroom_companion": "classroom_companion_agent",
    "material": "material_agent",
    "meeting_notes": "meeting_notes_agent",
    "router": "router_agent",
}


def init_agents() -> None:
    """通过 AgentFactory 统一创建所有 Agent 并缓存到 session_state。

    首次调用时创建 AgentFactory 实例，调用 create_all() 获取全部 6 个 Agent，
    分别存入 session_state 对应键名。后续调用直接复用缓存，不重复创建。

    依赖注入覆盖：
        - LLM / Settings → 构造函数 kwargs
        - KnowledgeBase / ConversationMemory / TeacherProfile → 属性赋值
        - OCR → 仅 WisdomTransferAgent
        - ASR → 仅 MeetingNotesAgent
    """
    if "agents" in st.session_state:
        return

    factory = AgentFactory()
    agents: dict[str, BaseAgent] = factory.create_all()

    # 缓存完整 agents 字典
    st.session_state.agents = agents

    # 同时为每个 Agent 设置独立键名，保持向后兼容
    for agent_type, session_key in _AGENT_KEY_MAP.items():
        st.session_state[session_key] = agents[agent_type]
...
def render_quick_lesson_prep() -> None:
    """渲染极速备课页面"""
    ...
    if st.button("开始备课", type="primary"):
        if not topic:
            ...
            with st.spinner("正在生成教案，请稍候..."):
                ...
                st.success("教案生成完成！")
                st.markdown("### 生成的教案")
                st.markdown(result.get("result", result))
...
def render_wisdom_transfer() -> None:
    """渲染智慧传承页面"""
        ...
        with col2:
            if st.button("开始识别与处理", type="primary"):
                with st.spinner("正在处理，请稍候..."):
                    ...
                    result = agent.run({"image_path": temp_path})
...
def render_classroom_companion() -> None:
    """渲染课堂伴教页面"""
    st.markdown("## 课堂实时伴教")
...
def render_material_recommend() -> None:
    """渲染素材推荐页面"""
    st.markdown("## 具象化素材推荐")

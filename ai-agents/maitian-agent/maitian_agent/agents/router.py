...
"""
        ...
        self._router_chain: Optional["RunnableSerializable"] = None
        self._validation_chain: Optional["RunnableSerializable"] = None
    ...
    def route(self, user_input: str, **kwargs) -> str:
        ...
        try:
            ...
            intent = router_chain.invoke(
                {"intent_list": intent_list, "user_input": user_input}
            ).strip()
    ...
    def run(self, input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        流程：意图识别 -> 二次校验 -> 保存路由决策到对话上下文
        """
        user_input = input_data.get("user_input", "")
        if not user_input:
            return self._handle_error(ValueError("缺少用户输入"))

        intent = self.route(user_input)
        intent_info = self.get_intent_info(intent)

        output = self._format_output(
            result={"intent": intent, "info": intent_info},
            metadata={"routed_intent": intent},
        )

        # 保存路由决策到对话上下文
        # _save_to_memory 期望 result 为字符串，此处序列化为可读文本
        memory_output = dict(output, result=f"路由意图: {intent}")
        self._save_to_memory(input_data, memory_output)

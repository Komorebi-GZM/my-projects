"""
[LEGACY] 麦田智囊 Streamlit 演示应用 — 早期版本

⚠️  此文件为项目初期的最小可运行演示，仅供概念验证。
    生产环境请使用 maitian_agent.api（FastAPI）或 maitian_agent.frontend（Streamlit + AgentFactory）。

与主项目的主要差异：
    - 直接实例化 Agent（主项目通过 AgentFactory 统一创建）
    - 无依赖注入、无 RAG/记忆/画像集成
    - 仅包含极速备课和智慧传承两个功能页面
"""

# 金融新闻舆情分析与市场预测 — 项目规范 (Project Convention)
...
│   ├── ch04_feature_engineering/      #    Ch04: 特征工程与相关性分析
│   ├── ch05_event_driven_strategy/    #    Ch05: 事件驱动策略分析
│   └── ch06_dashboard_summary/        #    Ch06: 可视化看板与总结报告
│
├── outputs/                           # 输出产物（每个章节独立子目录）
│   ├── ch01_data_preprocessing/       #    Ch01: 数据预处理产物
│   ├── ch02_descriptive_stats/        #    Ch02: 描述性统计与可视化产物
│   ├── ch03_text_mining_sentiment/    #    Ch03: 文本挖掘与情感分析产物
│   ├── ch04_feature_engineering/      #    Ch04: 特征工程与相关性分析产物
│   ├── ch05_event_driven_strategy/    #    Ch05: 事件驱动策略分析产物
│   └── ch06_dashboard_summary/        #    Ch06: 可视化看板与总结报告产物
│
├── docs/                              # 项目文档
│   ├── project_convention.md          #    项目规范（本文档）
│   ├── 金融新闻舆情分析与市场预测_流程设计.md  #    流程设计文档
│   ├── task_dispatch_guide.md         #    任务分发指南（依赖图 + 派活模板）
│   └── financial_news_sentiment_analysis_Execution_Prompts.md  #    各 Prompt 执行细节
│
├── logs/                              # 运行日志（自动生成，可清理）
├── .learnings/                        # AI 辅助开发经验记录（本地）
│
...
> **说明**：每个章节在 `src/` 下有对应的 `ch{NN}_{英文简称}/` 目录，在 `outputs/` 下有对应的 `ch{NN}_{英文全称}/` 目录。
> **注意**：原始数据文件名含空格（`stock_news_2016 to 2026.csv`），在脚本中引用时必须用引号包裹，例如：`pd.read_csv("data/stock_news_2016 to 2026.csv")`。
...
| 层级 | 格式 | 示例 |
|------|------|------|
| 章节脚本目录 | `src/ch{NN}_{英文简称}/` | `src/ch01_data_preprocessing/` |
| 章节输出目录 | `outputs/ch{NN}_{英文简称}/` | `outputs/ch01_data_preprocessing/` |

### 3.2 脚本命名

| 类型 | 格式 | 说明 |
|------|------|------|
| Python 脚本 | `{action}.py` | 如 `preprocess.py`, `analysis.py`, `predict.py` |
...
| 类型 | 格式 | 示例 |
|------|------|------|
| 数据文件 | `ch{NN}_{描述}.csv` | `ch02_news_category_distribution.csv` |
| 图片文件 | `ch{NN}_{描述}.png` | `ch02_sentiment_distribution.png` |
| 报告文件 | `ch{NN}_{描述}.md` | `ch02_descriptive_stats_report.md` |
| 模型文件 | `ch{NN}_{模型名}.pkl` | `ch05_event_model.pkl` |
...
> **数据**: stock_news_2016 to 2026.csv
...
```
【金融新闻舆情分析与市场预测 — 完整项目启动】

=== 第一步：了解项目 ===
阅读 docs/project_convention.md（目录结构、命名规则、禁止事项）
阅读 docs/金融新闻舆情分析与市场预测_流程设计.md 第一章（研究概述、数据概况、整体逻辑）
...
```
【金融新闻舆情分析与市场预测 — Prompt-{NN}: {章节名称}】

你现在阅读 docs/financial_news_sentiment_analysis_Execution_Prompts.md，概览任务状况，
你的任务是 Prompt-{NN}: {章节名称}；
执行标准看 docs/金融新闻舆情分析与市场预测_流程设计.md 第{N}章；
...
| Prompt | 章节名称 | 派活话术 |
|--------|----------|----------|
| Prompt-01 | 数据预处理 | `参考 docs/project_convention.md，执行 Prompt-01: 数据预处理` |
| Prompt-02 | 描述性统计与可视化 | `参考 docs/project_convention.md，执行 Prompt-02: 描述性统计与可视化` |
| Prompt-03 | 文本挖掘与情感分析 | `参考 docs/project_convention.md，执行 Prompt-03: 文本挖掘与情感分析` |
| Prompt-04 | 特征工程与相关性分析 | `参考 docs/project_convention.md，执行 Prompt-04: 特征工程与相关性分析` |
| Prompt-05 | 事件驱动策略分析 | `参考 docs/project_convention.md，执行 Prompt-05: 事件驱动策略分析` |
| Prompt-06 | 可视化看板与总结报告 | `参考 docs/project_convention.md，执行 Prompt-06: 可视化看板与总结报告` |
...
- [ ] **5. 已阅读本 Prompt 任务概述**: `docs/financial_news_sentiment_analysis_Execution_Prompts.md` 中搜索 `Prompt-{NN}`
- [ ] **6. 已阅读流程设计对应章节**: `docs/金融新闻舆情分析与市场预测_流程设计.md` 第{N}章（了解研究目标和方法选择依据）
...
#### ch01: 数据预处理
- [ ] 清洗后数据无 NaN 值（或缺失值已按策略合理处理）
- [ ] 新闻分类列（macro_government, stock_specific, geopolitical 等 16 个类别）编码正确
- [ ] 日期字段已解析为 datetime 类型，时间范围覆盖 2016-2026
- [ ] 数据类型正确（数值列为 float/int，分类列为 category，文本列为 str）
- [ ] 清洗后数据已保存为 `ch01_cleaned_data.csv`

#### ch02: 描述性统计与可视化
- [ ] 新闻数量按年份统计表已生成
- [ ] 各新闻分类（macro_government, stock_specific, geopolitical 等）的分布图已生成
- [ ] 新闻来源/媒体统计已生成
- [ ] 时间序列趋势图（新闻数量随时间变化）已生成
- [ ] 描述统计报告 `ch02_descriptive_stats_report.md` 已生成

#### ch03: 文本挖掘与情感分析
- [ ] 文本预处理（分词、去停用词、词干化）已完成
- [ ] 情感分析得分列已添加到数据中
- [ ] 情感得分分布图已生成
- [ ] 词频统计与词云图已生成
- [ ] 各新闻分类的情感得分对比分析已完成
- [ ] 情感分析结果数据 `ch03_sentiment_results.csv` 已保存

#### ch04: 特征工程与相关性分析
- [ ] 文本特征（TF-IDF、词嵌入等）已构建
- [ ] 时间特征（日期相关衍生变量）已构建
- [ ] 情感特征与股价特征的相关性分析已完成
- [ ] 特征重要性排序/相关性热力图已生成
- [ ] 特征数据集 `ch04_engineered_features.csv` 已保存

#### ch05: 事件驱动策略分析
- [ ] 不同新闻分类对股价的影响分析已完成
- [ ] 事件窗口（事件前/后 N 天）收益率分析已完成
- [ ] 各分类事件的平均收益率、胜率统计已生成
- [ ] 事件驱动策略回测结果已生成
- [ ] 策略分析报告 `ch05_event_strategy_report.md` 已生成

#### ch06: 可视化看板与总结报告
- [ ] 综合可视化看板（Dashboard）HTML 文件已生成
- [ ] 项目总结报告 `ch06_summary_report.md` 已生成
- [ ] 关键发现与结论已汇总
- [ ] 各章节核心图表已整合到看板中
- [ ] 看板可在浏览器中正常打开和交互
...
| # | 参数 | 值 |
|---|------|-----|
| 1 | PROJECT_NAME | `financial_news_sentiment_analysis` |
| 2 | PROJECT_DESCRIPTION | 金融新闻舆情分析与市场预测 |
| 3 | RAW_DATA_FILENAME | `stock_news_2016 to 2026.csv` |
| 4 | DATA_FORMAT | `csv` |
| 5 | ENTITY_NAME | 新闻 |
| 6 | ENTITY_CONFIG | macro_government, stock_specific, geopolitical, sector_banking_finance, sector_cement_infra, sector_energy_oil, sector_metals_mining, sector_it_tech, sector_pharma_healthcare, sector_auto, sector_retail_consumer, sector_telecom, rating_regulatory, research_rd, order_wins_deals, corporate_actions |
| 7 | CHAPTER_LIST | ch01 数据预处理, ch02 描述性统计与可视化, ch03 文本挖掘与情感分析, ch04 特征工程与相关性分析, ch05 事件驱动策略分析, ch06 可视化看板与总结报告 |
| 8 | CHAPTER_DEPENDENCIES | ch01 -> ch02 -> ch03 -> ch04 -> ch05 -> ch06（串行依赖） |
| 9 | PYTHON_VERSION | 3.10 |
| 10 | ENV_NAME / ENV_TYPE | py310 / conda |
